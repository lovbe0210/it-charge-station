...
class AudioComponent extends Card {
  static get cardName() {
    return "audio"
  }
  ...
  getLocales() {
    return this.editor.language.get("audio")
  }
  ...
  onBeforeRender = (action, url) => {
    const audioPlugin = this.editor.plugin.components.audio
    if (audioPlugin) {
  ...
  initPlayer() {
    ...
    if (this.container) {
      this.find(".data-audio-content").append(audio)
    }
  ...
  onActivate(activated) {
    ...
    if (activated) {
      this.addClass("data-audio-active")
    } else {
      this.removeClass("data-audio-active")
    }
  }

  checker(audioId, success, failed) {
  ...
  render() {
    ...
    if (!isEngine(this.editor)) {
      ...
      if (!audioId) {
        const errMessage = {message: "audio id is undefined"};
        throw errMessage;
      }
      this.checker(
        audioId,
  ...
  didRender() {
    ...
    this.on(isMobile ? "touchstart" : "click", () => {
