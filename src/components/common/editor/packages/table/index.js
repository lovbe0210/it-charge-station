    ...
    if (
      ...
      if (
        ...
        if (data.html && ~data.html.indexOf("<table")) {
          ...
          const findTableElement = element => {
    ...
    tables.each((_, index) => {
      ...
      tds.each((_, index) => {
        ...
        for (let i = 0, len = children.length; i < len; i++) {
          ...
          if (child?.name === "p" && child.children().length === 0) continue
      ...
      trs.each(child => {
        ...
        if (tds && tds?.length > 0) {
          const spans = tds.toArray().map(td => td[0].rowSpan)
