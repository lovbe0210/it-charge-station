...
class TableSelection extends EventEmitter2 {
  ...
  selectCol(begin, end = begin) {
    ...
    if (this.isShift) {
      ...
      if (this.beginAllCol) {
        ...
        if (this.tableModel) {
          const tdModel = this.tableModel.table[this.selectArea.begin.row][
    ...
    if (event instanceof MouseEvent && event.button === 2) {
      ...
      if (editableElement.length > 0) {
        ...
        if (block) {
          ...
          //节点不可见不处理
          if (
            (block.get()?.offsetTop || 0) + (block.get()?.clientHeight || 0) >
  ...
  getSelectionHtml(all = false) {
    ...
    for (let c = begin.col; c <= end.col; c++) {
      const colElement = colsEl.eq(c)?.get?.()
    ...
    for (let r = begin.row; r <= end.row; r++) {
      ...
      for (let _c2 = begin.col; _c2 <= end.col; _c2++) {
        ...
        if (tdClone) {
          ...
          const editoableElement = tdClone.firstChild
  ...
  showHighlight(area) {
    ...
    const headerHeight =
      this.colsHeader?.find(Template.COLS_HEADER_ITEM_CLASS).get?.()
        ?.offsetHeight || 0
    top += headerHeight
