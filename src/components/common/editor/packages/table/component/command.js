import {$, DATA_ID, DATA_TRANSIENT_ATTRIBUTES, isEngine, Parser, removeUnit} from '@aomao/engine';
import {EventEmitter2} from 'eventemitter2';
import Template from './template';

class TableCommand extends EventEmitter2 {
  // 构造函数中初始化成员变量
  ...
  insertColAt(index, count, isLeft, widths, ...args) {
    ...
    const baseColHeader = colsHeader.eq(colBase).get();
    if (!baseColHeader) return;
    ...
    const colsWidths = [];
    colsHeader.each((_, idx) => {
      const col = colsHeader.eq(idx);
    ...
    trs.each((tr, r) => {
      ...
      for (let i = 0; i < count; i++) {
        const td = tr.insertCell(insertIndex);
  ...
  insertCol(position, count = 1, ...args) {
    const { selection, wrapper } = this.table;
    const { tableModel } = selection;
    ...
    const width = colBars.eq(colBase).get().offsetWidth;

    this.insertColAt(insertCol, count, isLeft, width, ...args);
    if (isEnd && this.table.enableScroll) {
      const viewPort = this.viewport && this.viewport.get();
      if (!viewPort) return;
  ...
  removeCol(...args) {
    ...
    const selectArea = Object.assign({}, selection.getSelectArea());
        ...
        if (helper.isEmptyModelCol(tdModel)) {
          ...
          if (tdModel.isMulti) {
            ...
            if (cutHeader) {
              ...
              const td = trs.eq(r).get().insertCell(insertIndex);
              if (!td) return;
  ...
  insertRowAt(index, count, isUp, ...args) {
    ...
    const nodeId = this.editor.nodeId;
    const _loop = () => {
      const tr = this.tableRoot.get().insertRow(insertRow);
      if (!tr) return;
  ...
  insertRow(position, count = 1, ...args) {
  ...
  removeRow(...args) {
      ...
      trModel.forEach((tdModel, c) => {
        if (!helper.isEmptyModelCol(tdModel) && tdModel.isMulti && tdModel.rowSpan > 1) {
          if (r + tdModel.rowSpan - 1 > end.row) {
            ...
            const td = trs[end.row + 1].insertCell(insertIndex);
    ...
    for (let r = end.row; r >= begin.row; r--) {
      this.tableRoot.get().deleteRow(r);
  ...
  paste(data, ...args) {
    ...
    if (element.name === 'table') {
      ...
      if (!cell || helper.isEmptyModelCol(startCell) || helper.isEmptyModelCol(cell)) {
        return;
      }
      ...
      // 如果只选中一个非合并单元格，则延伸平铺，遇到表格边界自动增加行列
      if (isSingleTd) {
      ...
      selection.each((tdModel, r, c) => {
        const pasteR = (r - newArea.begin.row) % rowCount;
        const pasteC = (c - newArea.begin.col) % colCount;
        const pasteTd = pasteTableModel.table[pasteR][pasteC];

        if (!pasteTd) {
          return;
        }

        if (!helper.isEmptyModelCol(pasteTd) && pasteTd.isMulti && !helper.isEmptyModelCol(tdModel)) {
          const element = tdModel.element;
          element.rowSpan = Math.min(pasteTd.rowSpan, newArea.end.row - r + 1);
          element.colSpan = Math.min(pasteTd.colSpan, newArea.end.col - c + 1);
          helper.copyTo(pasteTd.element, element);
          return;
        }

        if (helper.isEmptyModelCol(pasteTd)) {
          if (!helper.isEmptyModelCol(tdModel)) {
            tdModel.element && tdModel.element.remove();
          }
          return;
        }
        ...
        if (pasteTd.element) {
          if (!helper.isEmptyModelCol(tdModel) && tdModel.element) {
            helper.copyTo(pasteTd.element, tdModel.element);
          }
        }
      });
    } else {
  ...
  mergeCell(...args) {
    ...
    const { begin, end } = selectArea;
    const rowCount = end.row - begin.row + 1;
    const colCount = end.col - begin.col + 1;
    let content = [];
    let mergeTd = null;
    this.splitCell();
    ...
    selection.each((tdModel, r, c) => {
      ...
      if (c === begin.col && r === begin.row) {
        ...
        mergeTd.rowSpan = rowCount;
        mergeTd.colSpan = colCount;
        return;
      }
      ...
      for (let r = rows - 1; r >= row; r--) {
        ...
        for (let c = cols - 1; c >= col; c--) {
          ...
          if (!helper.isEmptyModelCol(tdModel) && tdModel.isMulti) {
            ...
            const _td2 = tr.insertCell(_insertIndex2);
