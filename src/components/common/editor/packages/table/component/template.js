...
class Template {
   ...
   */
  htmlEdit({rows, cols, html, noBorder, overflow}, menus) {
    cols = cols === -Infinity ? 1 : cols
    rows = rows === -Infinity ? 1 : rows
    cols = cols === Infinity ? 10 : cols
    rows = rows === Infinity ? 10 : rows
    const tds = `<td ${DATA_TRANSIENT_ATTRIBUTES}="table-cell-selection">${this.getEmptyCell()}</td>`.repeat(
      cols
    )
  ...
  htmlView({html, noBorder, overflow}) {
