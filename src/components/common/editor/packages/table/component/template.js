...
// import { TableValue, TableMenu, TemplateInterface, TableInterface } from '../types';
...
class Template {
  // 静态常量，用于在代码中直接使用
  static TABLE_WRAPPER_CLASS = `.${TABLE_WRAPPER_CLASS_NAME}`;
  static TABLE_OVERFLOW_CLASS = `.${TABLE_OVERFLOW_CLASS_NAME}`;
  static TABLE_CLASS = `.${TABLE_CLASS_NAME}`;
  static COLS_HEADER_CLASS = `.${COLS_HEADER_CLASS_NAME}`;
  static COLS_HEADER_ITEM_CLASS = `.${COLS_HEADER_ITEM_CLASS_NAME}`;
  static COLS_HEADER_TRIGGER_CLASS = `.${COLS_HEADER_TRIGGER_CLASS_NAME}`;
  static COLS_ADDITION_HEADER_CLASS = `.${COLS_ADDITION_HEADER_CLASS_NAME}`;
  static ROWS_HEADER_CLASS = `.${ROWS_HEADER_CLASS_NAME}`;
  static ROWS_HEADER_ITEM_CLASS = `.${ROWS_HEADER_ITEM_CLASS_NAME}`;
  static ROWS_HEADER_TRIGGER_CLASS = `.${ROWS_HEADER_TRIGGER_CLASS_NAME}`;
  static HEADER_CLASS = `.${HEADER_CLASS_NAME}`;
  static MENUBAR_CLASS = `.${MENUBAR_CLASS_NAME}`;
  static MENUBAR_ITEM_CLASS = `.${MENUBAR_ITEM_CLASS_NAME}`;
  static MENUBAR_ITEM_INPUT_CALSS = `.${MENUBAR_ITEM_INPUT_CALSS_NAME}`;
  static VIEWPORT = `.${VIEWPORT}`;
  static VIEWPORT_READER = `.${VIEWPORT_READER}`;
  static PLACEHOLDER_CLASS = `.${PLACEHOLDER_CLASS_NAME}`;
  static MULTI_ADDITION_CLASS = `.${MULTI_ADDITION_CLASS_NAME}`;
  static TABLE_HIGHLIGHT_CLASS = `.${TABLE_HIGHLIGHT}`;
  static ROW_DELETE_BUTTON_CLASS = `.${ROW_DELETE_BUTTON_CLASS_NAME}`;
  static COL_DELETE_BUTTON_CLASS = `.${COL_DELETE_BUTTON_CLASS_NAME}`;
  static ROW_ADD_BUTTON_CLASS = `.${ROW_ADD_BUTTON_CLASS_NAME}`;
  static COL_ADD_BUTTON_CLASS = `.${COL_ADD_BUTTON_CLASS_NAME}`;
  static ROW_ADD_BUTTON_SPLIT_CLASS = `.${ROW_ADD_BUTTON_SPLIT_CLASS_NAME}`;
  static COL_ADD_BUTTON_SPLIT_CLASS = `.${COL_ADD_BUTTON_SPLIT_CLASS_NAME}`;
  static TABLE_TD_CONTENT_CLASS = `.${TABLE_TD_CONTENT_CLASS_NAME}`;
  static TABLE_TD_BG_CLASS = `.${TABLE_TD_BG_CLASS_NAME}`;
  static CellBG = `<div class="${TABLE_TD_BG_CLASS_NAME}"><div class="table-main-border-top"></div><div class="table-main-border-right"></div><div class="table-main-border-bottom"></div><div class="table-main-border-left"></div></div>`;

  // 将 isReadonly 作为普通属性
  isReadonly = false;

  // 静态方法，返回空单元格的 HTML 字符串
  static EmptyCell(readonly) {
    return `<div class="${TABLE_TD_CONTENT_CLASS_NAME}" ${DATA_TRANSIENT_ATTRIBUTES}="${DATA_CONTENTEDITABLE_KEY}" ${DATA_CONTENTEDITABLE_KEY}="${readonly ? 'false' : 'true'}" ${DATA_ELEMENT}="${EDITABLE}"><p><br /></p></div>${Template.CellBG}`;
  }
  ...
  /**
   * 用于Card渲染
   * @param value 参数对象，包含 rows、cols、html 等信息
   * @param menus 菜单数组
   * @return 返回 html 字符串
   */
  htmlEdit(value, menus) {
    let {rows, cols, html, noBorder, overflow} = value;
    ...
    let menuBar = menus.map(function (menu) {
      if (menu.split) {
      ...
      switch (menu.action) {
        ...
        case 'insertRowUp':
        case 'insertRowDown':
          menuContent = menuContent ? menuContent.replace(
            '$data',
    ...
    menuBar = [
      `<div ${DATA_ELEMENT}="${UI}" class="${MENUBAR_CLASS_NAME}">${menuBar.join('')}</div>`
    ];
    ...
    if (html) {
      ...
      const trsFound = normalTable.find('tr');
      rows = trsFound.length;
      html = normalTable.get ? normalTable.get().outerHTML : html;
    }

    const table = html || `<table class="${TABLE_CLASS_NAME}"${noBorder === true ? " data-table-no-border='true'" : ''} ${DATA_TRANSIENT_ATTRIBUTES}="class">${colgroup}${trs}</table>`;
  ...
  htmlView(value) {
    let {html, noBorder, overflow} = value;
    ...
    }>${table.get ? table.get().outerHTML : html}</div></div>`;
  }
}
