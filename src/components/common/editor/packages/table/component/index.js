...
class TableComponent extends Card {
  contenteditable = [`div${Template.TABLE_TD_CONTENT_CLASS}`]
        ...
        // },
        {
          ...
          onClick: () => {
            ...
            editor.messageSuccess(
              "copy",
              editor.language.get("copy", "success")
            )
          }
        },
        {
  ...
  initScrollbar() {
    ...
    this.scrollbar?.on("display", display => {
      if (display === "block") {
  ...
  remoteRefresh() {
    ...
    colElements.forEach((colElement, index) => {
      ...
      colItems.eq(index)?.css("width", `${Math.max(parseInt(width), this.colMinWidth)}px`)
    })
    ...
    rowElements.forEach((rowElement, index) => {
      rowItems.eq(index)?.css("height", Math.max(parseInt(rowElement.css("width")), this.rowMinHeight))
    })
    // this.conltrollBar.refresh();
    if (this.enableScroll) this.scrollbar?.refresh()
