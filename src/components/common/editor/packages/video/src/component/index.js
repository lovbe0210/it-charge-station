...
class VideoComponent extends Card {
  ...
  renderTemplate(value) {
    ...
    return `
        <div class="data-video">
			...
			${
      videoPlugin && (videoPlugin.options).showTitle !== false
        ? titleElement
        : ''
    }
        </div>
  ...
  onBeforeRender = (action, url) => {
    const videoPlugin =
      this.editor.plugin.findPlugin('video');
    if (videoPlugin) {
  ...
  toolbar() {
    ...
    const options =
      editor.plugin.findPlugin('video')?.options;
  ...
  getMaxWidth(node = this.getCenter()) {
  ...
  checker(videoId, success, failed) {
  ...
  render() {
    ...
    if (!isEngine(editor)) {
      ...
      this.container = $(this.renderTemplate(value));
      if (!videoId) {
        throw DOMException('video id is undefined');
      }
      ;
      this.checker(
        videoId,
      ...
      //设置为加载状态
      this.container = $(
        this.renderTemplate({...value, status})
      );
      command.executeMethod(
        'video-uploader',
        'query',
