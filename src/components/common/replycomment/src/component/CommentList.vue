<template>
  <div class="comment-list">
    <div v-for="comment in data" :key="comment.id">
      ...
      <div class="reply-box" v-if="comment.reply?.total > 0">
        <div class="reply-list">
          <ContentBox v-for="(reply, index) in replyList(comment.reply)"
                      :key="index"
                      :data="reply"
                      :parent-id="comment.id"
                      :contentBoxParam="param"/>
          <!-- 如果total>3则进行折叠，打开折叠后如果小于分页数，直接全部显示，否则使用分页显示 -->
          <div v-if="comment.reply.total > 3" class="fetch-more">
            <div v-if="collapse">
              <span>共{{ comment.reply.total }}条回复,&nbsp;</span>
          ...
          <div v-if="!collapse && comment.reply.total > pageSize" class="fetch-more">
            <a-pagination
              size="small"
              v-model="currentPage"
              :pageSize="pageSize"
              :total="comment.reply.total"
    ...
    methods: {
      ...
      submit({content, parentId, file, clear}) {
        ...
        setTimeout(() => {
          ...
          debugger
          if (comment) {
            let rawComment = this.data.find(v => v.id === parentId)
            if (rawComment) {
